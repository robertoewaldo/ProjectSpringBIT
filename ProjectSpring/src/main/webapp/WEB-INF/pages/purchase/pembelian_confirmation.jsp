...
<body>
	<h1>Konfirmasi Pembelian Voucher Isi Ulang</h1>
	<form:form method = "POST" action = "/ProjectSpring/pembelian/confirmed">
		<table>
			<tr>
				<td>Voucher:</td>
				<td>
					<select id="provider_input" name="provider" required="required">
						<option  value="${purchase.provider}">${purchase.provider}</option>
					</select>
				</td>
			</tr>
			<tr>
				<td>No Telp:</td>
				<td><input type='text' id="phone_input" class="input_number" name='phone_num' pattern="[08]{2}[0-9]{8,10}" required="required" readonly="readonly" value="${purchase.phone_num}"/></td>
			</tr>
		</table>
		
		<input name="submit" id="submit_btn" type="submit" value="confirm" />
